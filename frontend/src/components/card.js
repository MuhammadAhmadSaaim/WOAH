...
const Card = ({ name, price, description, category, image, userid, bidActive }) => {
  ...
  const highestBidder = async () => {
    const response = await fetch(`${window.location.origin}/bid/highestBidder`, {
      ...
      body: JSON.stringify({ name: name, price: price, description: description, category: category }),
    });
    const data = await response.json();
        ...
        <div className="custom-modal" onClick={closeModal} style={{ zIndex: 1 }}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <input
              ...
              type="number"
              placeholder={`Minimum bid: Rs ${bid + 1}`}
              min={price + 1}
